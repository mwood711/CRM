...
<h1>Add Faculty to <?php echo $description; ?></h1>
<small></small>
</div>
</section>
<!-- Main content -->
...
<div class="card-body">
<form class="col-sm-6" action="addFacultyAtEventQuery.php" method="POST">

<div class="form-group">
<label>Select Faculty Member</label>
<select class="form-control" name="faculty_id" id="inputGroupSelect">
     ...
     <?php
          ...
          while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ){
              ...
              echo "<option selected value='".$row['faculty_id']."'>".$row['fname']." ".$row['lname']."</option>";
...
<?php 
    ...
    require('dbconfig.php');
    $sql = "SELECT TOP 1 * FROM FacultyAtEvent ORDER BY faculty_event_id DESC";
    $stmt = sqlsrv_query( $conn, $sql );
    if( $stmt === false) {
...
<input type="hidden" name="faculty_event_id" value="<?php echo $faculty_event_id ?>"/>
